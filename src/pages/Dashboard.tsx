...
export default function Dashboard() {
  ...
  async function loadAll() {
    if (!user) return;
    const [{ data: tk }, { count: msgCount }, { count: evCount }, { count: connCount }, { data: prof }] = await Promise.all([
    ...
    // Activity: latest tasks + event registrations
    ...
    (tk ?? []).slice(0, 5).forEach((t: Task) => items.push({ kind: "Task", text: t.title, at: t.created_at }));
    (regs ?? []).forEach((r: any) =>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 panel p-5">
          ...
          <form onSubmit={addTask} className="flex gap-2 mb-4">
            ...
            />
            <button type="submit" className="h-9 px-3 rounded-md bg-primary text-primary-foreground text-sm font-medium inline-flex items-center gap-1.5 hover:opacity-90">
          ...
          <div className="panel p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setMonth(subMonths(month, 1))} className="h-7 w-7 grid place-items-center rounded-md hover:bg-[hsl(var(--surface-3))] text-muted-foreground">‹</button>
                <button onClick={() => setMonth(addMonths(month, 1))} className="h-7 w-7 grid place-items-center rounded-md hover:bg-[hsl(var(--surface-3))] text-muted-foreground">›</button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {days.map((d) => {
                ...
                return (
                  <div
                    key={iso}
                    className={`aspect-square text-xs grid place-items-center rounded-md ${today ? "bg-primary text-primary-foreground" : isSameMonth(d, month) ? "text-foreground" : "text-muted-foreground"} ${hasTask && !today ? "ring-1 ring-primary/40" : ""}`}
                  >
                    {format(d, "d")}
                  </div>
                );
              ...
              {activity.map((a, i) => (
                <li key={i} className="flex items-start gap-3 text-sm">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">{format(new Date(a.at), "MMM d, h:mm a")}</div>
                  </div>
                </li>
              ))}
            </ul>
